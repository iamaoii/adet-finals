...
<svg width="800" height="1000" viewBox="0 0 800 1000" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Invoice Meta Column (Omitted Invoice Number and Due Date) -->
  <text x="450" y="170" class="section-title">INVOICE DETAILS</text>

  <text x="450" y="195" class="meta-label">Invoice Date</text>
  <text x="450" y="215" class="meta-value">05/20/2026</text>

  <text x="620" y="195" class="meta-label">Category</text>
  <text x="620" y="215" class="meta-value">Services</text>
  ...
  <text x="70" y="775" class="body-bold">Account Name: Apex Tech Solutions</text>
  <text x="70" y="795" class="body-text">Bank Transfer to Bank of the Philippine Islands (BPI)</text>
  <text x="70" y="815" class="company-sub">Note: Please quote your project reference when making bank transfer payments.</text>
...
async function main() {
  ...
  const outputPathComplete = path.join(clientPublicDir, 'invoice_sample.png');
  const outputPathPending  = path.join(clientPublicDir, 'invoice_pending_sample.png');

  try {
    console.log(`Rendering Complete SVG to PNG: ${outputPathComplete}...`);
    await sharp(Buffer.from(svgContentComplete))
      .png()
      .toFile(outputPathComplete);
    console.log('Success! Complete sample invoice generated.');

    console.log(`Rendering Pending SVG to PNG: ${outputPathPending}...`);
    await sharp(Buffer.from(svgContentPending))
      .png()
      .toFile(outputPathPending);
    console.log('Success! Pending sample invoice generated.');
  } catch (err) {
